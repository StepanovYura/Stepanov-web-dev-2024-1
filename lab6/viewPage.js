    ...
    filterButtons.forEach(button => {
        button.addEventListener('click', () => {
            ...
            if (lastFilter.category === category && lastFilter.kind === kind) {
                ...
                button.checked = false; // Убираем отметку
            } else {
                // Применяем фильтр
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Функция для обновления состояния поля времени доставки
    const updateDeliveryTimeField = () => {
        if (fastDelivery.checked) {
            deliveryTime.removeAttribute('required');
            deliveryTime.disabled = true; // Отключаем поле времени
            deliveryTime.value = ''; // Очищаем значение
        } else if (accuracyDelivery.checked) {
            deliveryTime.setAttribute('required', 'required');
            deliveryTime.disabled = false; // Включаем поле времени
        }
    };
